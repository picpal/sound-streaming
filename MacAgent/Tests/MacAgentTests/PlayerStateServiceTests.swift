...
final class PlayerStateServiceTests: XCTestCase {
    private func snap(id: String, paused: Bool = false, hasVideo: Bool = true, position: Double = 0) -> YTMSnapshot {
        YTMSnapshot(videoId: id, title: "T-\(id)", byline: "A • Al", paused: paused,
                    position: position, duration: 100, hasVideo: hasVideo)
    }
    ...
    func testIngestIdenticalSnapshotNoBump() {
        ...
        _ = s.ingest(snap(id: "a", position: 42), now: Date(timeIntervalSince1970: 1))
